...
function renderRoutes(solution) {
  ...
  const visitByIdMap = new Map(
    solution.visits.map((visit) => [visit.id, visit]),
  );
  for (let vehicle of solution.vehicles) {
    const homeLocation = vehicle.homeLocation;
    const locations = vehicle.visits.map(
      (visitId) => visitByIdMap.get(visitId).location,
    );
...
function renderTimelines(routePlan) {
  ...
  $.each(routePlan.visits, function (index, visit) {
    ...
    const visitGroupElement = $(`<div/>`).append(
      $(`<h5 class="card-title mb-1"/>`).text(`${visit.name}`),
    );
